# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/torrentPlayer/lib/TaskView2.py
from Plugins.Extensions.torrentPlayer.lib.gimports import *
from Plugins.Extensions.torrentPlayer.lib.pltools import printD,printE
...
def printD(label='', Ddata='' ):
        log_file='/tmp/torrentPlayer/torrentPlayer4.dlog'  
        Ddata=str(Ddata)
        label=str(label)
        import traceback
...
class JobViewNew(InfoBarNotifications, Screen, ConfigListScreen):
    ...
    def __init__(self, session, job, parent = None, cancelable = True, backgroundable = True, afterEventChangeable = True):
        ...
        try:
            ...
            self.size = 'Size ' + str(int(txt) / 1048576) + 'MB'
        except:
            self.size = '000'
    ...
    def abort(self):
        if self.job.status == self.job.NOT_STARTED:
            ...
            self.session.openWithCallback(self.stop_job, MessageBox, _('Stop download now,downloaded movie will be deleted.'), MessageBox.TYPE_YESNO)
        else:
            self.close(False)
    ...
    def stop_job(self, result):
        ...
        if result:
            ...
            try:
               self.job.cancel()
               downloadfolder = config.torrentPlayer.downloadlocation.value

                
                printD('downloadfolder +"/"+ self.job.name',downloadfolder +"/"+ self.job.name)
