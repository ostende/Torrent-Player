...
from Plugins.Extensions.torrentPlayer.lib.pltools import trace_error,logdata,removeunicode,removeColorCode
from . import _, __author__, __version__, __email__
log_file='/tmp/torrentPlayer/subsupport.log' 
def printE(msg=''):
...
logdata=printD
trace_error=printE
logdata("subs started","started")
def removeDate(s):

    try:
        import dateutil.parser as dparser

        mdate= dparser.parse(s,fuzzy=True)
        myear=mdate.split("-")[0]
        s=s.split(myear).replace("-",' ')+" "+myear
    except:
        trace_error()
        return s
def cleanSRTitle(title):
        try:
           title=removeunicode(title)
           title=removeColorCode(title)
        except Exception as error:
            logdata("clean error",error)
            pass
...
def getImdb_id():
    try:
     tmbd_id=open("/tmp/torrentPlayer/imdb_id").read()
     return tmbd_id
    except:
      trace_error()  
      return None  
...
class SubsSupport(SubsSupportEmbedded):
    ...
    def __init__(self, session=None, subsPath=None, defaultPath=None, forceDefaultPath=False, autoLoad=True,
                 showGUIInfoMessages=True, embeddedSupport=False, preferEmbedded=False, searchSupport=False, configEntry=None,playerSearchParams={},subsSupportCB=None):
        if session is not None:
            self.session = session
        self.searchSupport = searchSupport
        self.playerSearchParams=playerSearchParams#searchParams={"title":FileName,"url":filesrcLocation,"imdb_id":imdb_id,"subFile":subFile}
        self.playerSearchParams=playerSearchParams
        self.srefURL=playerSearchParams.get("url",'')
        self.srefTitle=playerSearchParams.get("title",'')
        self.srefTitle=cleanSRTitle(self.srefTitle)
        
        self.imdb_id=playerSearchParams.get("imdb",'')
        self.srefSubfile=playerSearchParams.get("subFile",'')
        logdata("susbsupport started",self.playerSearchParams)



        self.subsSupportCB=subsSupportCB
    ...
    def subsMenu(self,title='',url=''):
         
        #self.subsSupportCB=subsSupportCB
         
        if not self.subsSupportCB=={}:
            
            self.subsSupportCB('subsStart')    
            
           
            self.session.openWithCallback(self.__subsMenuCB, SubsMenu, self,
                                          self.__subsPath, self.__subsDir, self.__subsEnc, self.embeddedSupport, self.embeddedEnabled, self.searchSupport,playerSearchParams=self.playerSearchParams,subsSupportCB=self.subsSupportCB)
...
def PanelListEntry(name, mode):
    res = [(name, mode)]
    if isFullHD():
      res.append(MultiContentEntryText(pos=(5, 5), size=(950, 40), font=0, flags=RT_VALIGN_CENTER, text=name))
    else:
      res.append(MultiContentEntryText(pos=(5, 5), size=(750, 25), font=0, flags=RT_VALIGN_CENTER, text=name))
    return res
...
class SubsMenu(Screen):
    if isFullHD():
        skin = """
            <screen position="center,center" size="1000,900" zPosition="1" >
                <widget name="title_label" position="0,5" size="750,45" valign="center" halign="center" font="Regular;38" transparent="1" foregroundColor="white" />
                <widget name="subfile_label" position="0,50" size="1000,70" valign="center" halign="center" font="Regular;33" transparent="1" foregroundColor="#DAA520" />
                <widget name="subfile_list" position="center,100" size="900,45" transparent="1" />
                <eLabel position="5,202" size="735,1" backgroundColor="#999999" />
                <widget name="menu_list" position="0,210" size="900,700" itemHeight="60" transparent="1" scrollbarMode="showOnDemand" />
        ...
        skin = """
            <screen position="center,center" size="750,600" zPosition="1" >
                ...
                <widget name="subfile_list" position="center,100" size="750,30" transparent="1" />
                <eLabel position="5,135" size="490,1" backgroundColor="#999999" />
                <widget name="menu_list" position="0,140" size="750,450" transparent="1" scrollbarMode="showOnDemand" />
    ...
    def __init__(self, session, infobar, subfile=None, subdir=None, encoding=None, embeddedSupport=False, embeddedEnabled=False, searchSupport=False,playerSearchParams={},subsSupportCB=None):
        Screen.__init__(self, session)
        self.infobar = infobar
        self.subfile = subfile
        self.subdir = subdir

        self.subsSettings = initSubsSettings(None)
    ...
    def showMediaLists(self):
            from Plugins.Extensions.torrentPlayer.screens.imdbTools import imdbAPI
            from Plugins.Extensions.torrentPlayer.lib.pltools import getMediaShortName
            self.imdbAPIinst=imdbAPI()
            logdata("self.srefTitle",self.srefTitle)
            srName, year, tvshow, season, episode=getMediaShortName(self.srefTitle)
            logdata("short sname",srName)   
            itemsList = self.imdbAPIinst.searchMulti(srName)
            logdata("itemsList",itemsList)
            logdata('allparams',(srName, year, tvshow, season, episode))
            if len(itemsList)==0:
                
                   itemsList.append((srName,year,tvshow,season,episode))
            self.selectMovie(itemsList = itemsList)


    def selectMovie(self, itemsList = []):
            ...
            for items in itemsList:
                ...
                # (u'Tangled Ever After', u'2012-01-13', u'movie', 82881)
                title=str(items[0])
                try:year=str(items[1].split("-")[0])
                except:year=str(items[1])
                mtype=str(items[2])
                nlines.append((title+"-"+year+"-"+mtype,items))
            self.session.openWithCallback(self.stchoicesback, ChoiceBox, 'Confirm movie name', nlines)


    def stchoicesback(self, itemsList = []):
        
       
        #itemsList=[(u'Fantasy Island', u'2020-02-12', u'movie', 539537)
        if itemsList>0:
            self.resetSearchParams=False
            try:
              imdb_id = self.imdbAPIinst.getMovieIMDBIDfromID(itemsList[1][3])
            except:
              imdb_id=self.imdb_id
            self.selectedImdbID=imdb_id
            self.selectedName=str(itemsList[0])
            self.selectedType=str(itemsList[1][2])
            try:self.selectedYear=str(itemsList[1][1].split("-")[0])
            except:self.selectedYear=itemsList[1][1]
            
            subsSettings=self.infobar.subsSettings
            try: title=self.selectedName.split("-")[0]
            except:title=self.selectedName
            subsSettings.search.title.value=self.selectedName.split("-")[0]
            subsSettings.search.title.save()
            logdata('self.selectedYear)',self.selectedYear)
            try:subsSettings.search.year.value=int(self.selectedYear)
            except:subsSettings.search.year.value=int("0000")
            subsSettings.search.year.save()
            subsSettings.search.type.value=self.selectedType
            subsSettings.search.type.save()
            self.initMenu(imdb_id,title)

        else:
            self.cancel()
   
    def getSupportedSubids(self,imdb_id=''):
        

         
        if imdb_id:
            from Plugins.Extensions.torrentPlayer.lib.subsapi import getSupportedSub_ids
            
            sts,SupportedSub_ids=getSupportedSub_ids(imdb_id,default_languages=True)
            return SupportedSub_ids
        else:
    ...
    def getRelatedSubFiles(self,srURL,srTtitle):
                def findPattarn(pattern, path,srTtitle,torrent=False):
                    import os
                    result = []
                    ...
                    for file in os.listdir(path):
                        if file.endswith(".srt"):
                            ...
                            srtPath=os.path.join(path, file)
                            
                            if torrent:
                               result.append((file,srtPath))
                            else:   
                              if srTtitle in srtPath:
                                
                                  result.append((file,srtPath))
                    return result

        
                srtFiles=findPattarn(".srt","/tmp",srTtitle)
                srtFiles +=findPattarn(".srt","/media/hdd/",srTtitle)
                srtFiles +=findPattarn(".srt","/media/hdd/movies/",srTtitle)
                srtFiles +=findPattarn(".srt","/media/hdd/torrentPlayer/subtitles/",srTtitle)
                
                
                srtPath=os.path.dirname(srURL)
                if os.path.exists(srtPath+"/torrent.nfo"):
                        
                        torrent=True              
                        srtFiles +=findPattarn(".srt",srtPath,srTtitle,torrent)
                        return srtFiles
                else:
    ...
    def initMenu(self,imdb_id='',shortName=''):

        try:        
            if shortName=='':
               shortName =self.srefTitle
            self.menu=[]
            logdata("initMenu-started-shortName",shortName)
         
            supportedSub_ids=self.getSupportedSubids(imdb_id)         
            supportedSub_ids.sort()
            for sub_id in supportedSub_ids:
               self.menu.append((str(sub_id[0]), "sub_id=%s"%sub_id[1])) 
     
            if True:

                
               ##get related srt files in default directories
                subsFilesList=self.getRelatedSubFiles(self.srefURL,shortName)
                for item in subsFilesList:
                   file=item[0]
                   path=item[1]
                   self.menu.append((file,path))
                ##append original subssupport menu    
                self.menu.append((_('Choose subtitles'), 'choose'))
            ...
            logdata("Error 3",error)
            trace_error()
    def ok(self):
        mode = self["menu_list"].getCurrent()[0][1]
        title = self["menu_list"].getCurrent()[0][0]
        ...
        if mode == 'choose':
            ...
            self.cancel()
             ...
             if self.selectedImdbID!='':
                self.imdb_id =self.selectedImdbID
             if self.imdb_id=="":
                imdb_id=getImdb_id()
             else:
                 imdb_id=self.imdb_id
             ...
             if imdb_id:
                from Plugins.Extensions.torrentPlayer.lib.subsapi import getmovie_sub 
                sts,sub_url,subPath=getmovie_sub(imdb_id,sub_id)
                
                if sts:
                              
                   self.subsChooserCB( subfile=subPath, embeddedSubtitle=None, forceReload=False) 
            
        else:
           self.subsChooserCB( subfile=mode, embeddedSubtitle=None, forceReload=False)
    ...
    def searchSubs(self):
        def checkDownloadedSubsSelection(downloadedSubtitle=None):
            if downloadedSubtitle:
                ...
                encoding=ENCODINGS[self.subsSettings.encodingsGroup.getValue()]
                

                
                if  self.subsSupportCB:
                    
                   self.subsSupportCB(downloadedSubtitle,encoding)
        
        def paramsDialogCB(callback=None):
            if callback:
                self.session.openWithCallback(checkDownloadedSubsSelection, SubsSearch, seeker, subsSettings.search, self.srefTitle, titleList, resetSearchParams=False)
        ...
        def showProvidersErrorCB(callback):
            ...
            if subsSettings.search.openParamsDialogOnSearch.value:
                ...
                self.session.openWithCallback(checkDownloadedSubsSelection, SubsSearch, seeker, subsSettings.search, self.srefTitle, titleList)

        if self.searchSupport:
            
            
            srURL=self.srefURL
            srTitle=self.srefTitle

            
            titleList = self.getSearchTitleList(srTitle, srURL)
            logdata("titleList",titleList)
            subsSettings = self.infobar.subsSettings
            ...
            if seeker.providers_error and subsSettings.search.showProvidersErrorMessage.value:
                ...
                self.session.openWithCallback(paramsDialogCB, SubsSearchParamsMenu, seeker, subsSettings.search, titleList, enabledList=False,resetSearchParams=self.resetSearchParams,playerSearchParams=self.playerSearchParams)
            else:
                                self.session.openWithCallback(checkDownloadedSubsSelection, SubsSearch, seeker, subsSettings.search, srURL, titleList)
    def copyTorrentSRT(self,subfile):
        ...
        try:
           ...
           subdir = os.path.dirname(sPath)
           if not os.path.exists(subdir+"/torrent.nfo" ):
              return False                   
           import shutil
            
            
           if os.path.exists(subfile):
               if os.path.exists(default_subFile):
                   backFile=default_subFile[:-4]+"-backup.srt"
                   if not os.path.exists(backFile):
                      shutil.move(default_subFile,backFile)
                
               shutil.copyfile(subfile, default_subFile)
        except Exception as e:
               
               printE()                  
                              
    def subsChooserCB(self, subfile=None, embeddedSubtitle=None, forceReload=False):
        subdir = os.path.dirname(self.srefURL)
        if subdir.startswith("/") and  os.path.exists(subdir+"/torrent.nfo"):
                  
          self.copyTorrentSRT(subfile)
        ...
        if subfile is not None and self.subfile != subfile:
            ...
            if self.subsSupportCB:
               self.subsSupportCB(subfile,encoding) 
            self.cancel()
    ...
    def __init__(self, session, subsSettings, subdir=None, embeddedSupport=False, searchSupport=False, historySupport=False, titleList=None):
        ...
        if embeddedSupport:
	        ...
	        subtitle = service and service.subtitleTracks()
                self.embeddedList = subtitle.getNumberOfSubtitleTracks()
            except:
...
class SubsEmbeddedSelection(Screen):
    ...
    def getSubtitleList(self):
        ...
        try:
            subtitle = service and service.subtitleTracks()
            subtitlelist = subtitle.getNumberOfSubtitleTracks()	
        except:
	    subtitle = service and service.subtitle()
            subtitlelist = subtitle and subtitle.getSubtitleList()
        embeddedlist = []
        for x in str(subtitlelist):
            if x[0] == 2:
                types = ("unknown", "embedded", "SSA file", "ASS file",
                            "SRT file", "VOB file", "PGS file")
...
class SubsSearch(Screen):
    if isFullHD():
        ...
        <screen name="SubsSearch" position="center,center" size="1350,780" zPosition="3" >
            ...
            <ePixmap pixmap="skin_default/buttons/key_yelltorrentow.png" position="692,727" size="35,25" transparent="1" alphatest="on" />
...
class SubsSearchParamsMenu(Screen, ConfigListScreen):
    ...
    def __init__(self, session, seeker, searchSettings, titleList=None, resetSearchParams=True, enabledList=True, windowTitle=None,playerSearchParams={}):
        ratio               = 1.5 if isFullHD() else 1
        xOffset             = 10
        desktopSize         = getDesktopSize()
        ...
        Screen.__init__(self, session)
        ConfigListScreen.__init__(self, [])##mfaraj
        self["config"] = MyConfigList([], None, enabledList)
        ...
        self.sourceTitle=self.searchSettings.title.value
        ...
        self["configActions"] = ActionMap(["OkCancelActions", "ColorActions", "DirectionActions","SubtitlesActions"],
            {
                "ok": self.keyOK,
                "cancel": self.keyCancel,
    ...
    def buildMenu(self):
        menuList = []
        menuList.append(getConfigListEntry(_("Title(press text to edit") , self.searchSettings.title))
        menuList.append(getConfigListEntry(_("Type") , self.searchSettings.type))
         
        if self.searchSettings.type.value.strip() == "movie":
    ...
    def switchToConfigList(self):#mfaraj
    ...
    def getNewVK(self):
        if self["config"].getCurrent()[1] == self.searchSettings.title: 
            ...
            self["configActions"].setEnabled(True)
            self.currentList = self.LIST_CONFIG
        from Plugins.Extensions.torrentPlayer.lib.NewVirtualKeyBoard import VirtualKeyBoard
        self.session.openWithCallback(self.searchCallback, VirtualKeyBoard, title=_('Confirm media title!'), text=self.searchSettings.title.value)



        
    def  searchCallback(self,ret=None):#mfaraj
         if ret:
             self.searchSettings.title.value=ret
             self.buildMenu()
